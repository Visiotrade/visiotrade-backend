...
async function sendeEmail(to, subject, html, pdfBuffer = null) {
  ...
  if (pdfBuffer) {
    body.attachment = [{ name: 'Rechnung_VisioTrade.pdf', content: pdfBuffer.toString('base64') }];
  }
  ...
  if (!res.ok) { const err = await res.text(); throw new Error(`Brevo Fehler: ${err}`); }
  return res.json();
}
...
async function erstelleLexofficeRechnung(bestellung, positionen) {
  ...
  const rechnung = {
    ...
    address: { name: bestellung.kundenname, street: bestellung.lieferadresse_strasse || '', zip: bestellung.lieferadresse_plz || '', city: bestellung.lieferadresse_ort || '', countryCode: 'DE' },
    lineItems: positionen.map(pos => ({
      type: 'custom', name: pos.produktname, quantity: pos.menge_m2_gesamt, unitName: 'm²',
      unitPrice: { currency: 'EUR', netAmount: Math.round((pos.preis_je_paket / (pos.menge_m2_gesamt / pos.anzahl_pakete)) * 10000) / 10000, taxRatePercentage: 19 },
  ...
  const renderRes = await fetch(`https://api.lexoffice.io/v1/invoices/${result.id}/document`, {
    method: 'GET',
    headers: { 'Authorization': `Bearer ${process.env.LEXOFFICE_API_KEY}`, 'Accept': 'application/json' }
  });
  let pdfBuffer = null;
  if (renderRes.ok) {
    ...
    const pdfRes = await fetch(`https://api.lexoffice.io/v1/files/${documentFileId}`, {
      headers: { 'Authorization': `Bearer ${process.env.LEXOFFICE_API_KEY}`, 'Accept': 'application/pdf' }
    });
    if (pdfRes.ok) { pdfBuffer = await pdfRes.buffer(); console.log(`✅ PDF heruntergeladen (${pdfBuffer.length} bytes)`); }
    else { console.error(`❌ PDF Download Fehler: ${pdfRes.status}`); }
  } else { console.error(`❌ PDF Rendering Fehler: ${renderRes.status} - ${await renderRes.text()}`); }
...
async function bestellungAbwickeln(bestellungId, zahlungsId) {
  ...
  try {
    ...
    if (!bestellungen || !Array.isArray(bestellungen) || !bestellungen[0]) { console.error(`❌ Bestellung ${bestellungId} nicht gefunden`); return; }
    ...
    let pdfBuffer = null, lexofficeId = null;
    if (process.env.LEXOFFICE_API_KEY) {
      ...
      if (lexResult) { lexofficeId = lexResult.id; pdfBuffer = lexResult.pdfBuffer; await supabaseUpdate('bestellungen', bestellungId, { lexoffice_id: lexofficeId }); console.log(`✅ Lexoffice Rechnung erstellt: ${lexofficeId}`); }
    }
    const netto = parseFloat(bestellung.gesamtbetrag);
    const emailHtml = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;"><div style="background:#3D2B1F;padding:20px;text-align:center;"><h1 style="color:#C49A2B;font-family:Georgia,serif;margin:0;">VisioTrade</h1><p style="color:white;margin:5px 0 0;">Premium Parkett</p></div><div style="padding:30px;background:#FAF7F2;"><h2 style="color:#3D2B1F;">Vielen Dank für Ihre Bestellung!</h2><p>Sehr geehrte/r ${bestellung.kundenname},</p><p>Ihre Bestellung wurde erfolgreich aufgenommen und bezahlt.</p><table style="width:100%;border-collapse:collapse;margin:20px 0;"><tr style="background:#F5EDD8;"><td style="padding:10px;font-weight:bold;">Bestellnummer</td><td style="padding:10px;">#${bestellungId}</td></tr><tr><td style="padding:10px;font-weight:bold;">Nettobetrag</td><td style="padding:10px;">${netto.toFixed(2).replace('.',',')} €</td></tr><tr style="background:#F5EDD8;"><td style="padding:10px;font-weight:bold;">MwSt. 19%</td><td style="padding:10px;">${(netto*0.19).toFixed(2).replace('.',',')} €</td></tr><tr><td style="padding:10px;font-weight:bold;font-size:16px;">Gesamtbetrag</td><td style="padding:10px;font-weight:bold;font-size:16px;color:#8B6914;">${(netto*1.19).toFixed(2).replace('.',',')} €</td></tr><tr style="background:#F5EDD8;"><td style="padding:10px;font-weight:bold;">Lieferart</td><td style="padding:10px;">${bestellung.lieferart==='abholung'?'Selbstabholung':'Lieferung'}</td></tr></table>${pdfBuffer?'<p>Ihre Rechnung finden Sie im Anhang dieser Email.</p>':''}<p style="color:#6B7280;font-size:13px;margin-top:30px;">Bei Fragen stehen wir Ihnen gerne zur Verfügung.<br>VisioTrade GmbH</p></div></div>`;
    if (bestellung.email) {
      console.log(`🔄 Sende Email an ${bestellung.email}...`);
      try { await sendeEmail(bestellung.email, `Ihre Bestellung #${bestellungId} bei VisioTrade — Bestätigung & Rechnung`, emailHtml, pdfBuffer); console.log(`✅ Email gesendet an ${bestellung.email}`); }
      catch (emailErr) { console.error(`❌ Email Fehler:`, emailErr.message); }
    }
  ...
  } catch (err) { console.error(`❌ Fehler bei Bestellabwicklung:`, err); }
}
...
app.post('/api/stripe/create-session', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: items.map(item => ({ price_data: { currency: 'eur', product_data: { name: item.name, description: `${item.gesamt_m2} m²` }, unit_amount: Math.round(item.preis * 119) }, quantity: 1 })),
      mode: 'payment',
...
app.post('/webhook/stripe', async (req, res) => {
  ...
  } catch (err) { console.error('❌ Webhook Parse Fehler:', err.message); return res.status(400).send(`Webhook Error: ${err.message}`); }
...
app.post('/api/paypal/create-order', async (req, res) => {
  try {
    ...
    console.log(`🔄 PayPal create-order für Bestellung ${bestellung_id}, netto: ${netto}`);
...
app.post('/api/paypal/capture/:orderId', async (req, res) => {
  try {
    ...
    if (captureData.status === 'COMPLETED') { await bestellungAbwickeln(parseInt(bestellung_id), orderId); }
    res.json({ status: captureData.status });
  } catch (err) { console.error('❌ PayPal Capture Error:', err); res.status(500).json({ error: err.message }); }
});
